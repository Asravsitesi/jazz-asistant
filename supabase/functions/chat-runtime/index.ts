...
const pools={
  ...
  notion:[1,2,3,4,5].map(index=>({index,key:(Deno.env.get(`NOTION_API_KEY_${index}`)||'').trim()})).filter(x=>x.key)
};
...
async function resolveModel(provider,key){
  ...
  try{
    ...
    if(r.ok){
      const body=await r.json(),ids=(body.data||[]).filter(x=>x.active!==false).map(x=>x.id).filter(Boolean);
      ...
      const model=preferred.find(x=>ids.includes(x))||ids.find(x=>!/whisper|tts|speech|guard|safeguard|prompt-guard/i.test(x));
      if(model){modelCache.set(cacheKey,model);return model;}
    }
  }catch{}
  return endpoints.groq[1];
}
async function compatible(provider,key,messages,maxTokens){
...
async function gemini(key,messages,maxTokens){
  ...
  return {answer:body.candidates?.[0]?.content?.parts?.map(x=>x.text||'').join('')||'',model,usage:body.usageMetadata||{}};
}
async function testModel(provider,key,index){
  const started=Date.now();
  try{
    const messages=[{role:'user',content:'Reply only with OK.'}];
    if(provider==='gemini')await gemini(key,messages,8);else await compatible(provider,key,messages,8);
    return {provider,index,configured:true,ok:true,latencyMs:Date.now()-started};
...
async function testNotion(key,index){
  const started=Date.now();
  try{
    const r=await fetch('https:'+'//api.notion.com/v1/users/me',{signal:AbortSignal.timeout(15000),headers:{'Authorization':'Bearer '+key,'Notion-Version':'2022-06-28'}});
    return {provider:'notion',index,configured:true,ok:r.ok,status:r.status,latencyMs:Date.now()-started};
  }catch{return {provider:'notion',index,configured:true,ok:false,status:0,latencyMs:Date.now()-started};}
}
async function diagnostics(){
  const jobs=[];
  pools.notion.forEach(x=>jobs.push(testNotion(x.key,x.index)));
  for(const provider of ['groq','openrouter','openai','gemini'])pools[provider].forEach((key,i)=>jobs.push(testModel(provider,key,i+1)));
  const results=await Promise.all(jobs);
  return {configured:{groq:pools.groq.length,openrouter:pools.openrouter.length,openai:pools.openai.length,gemini:pools.gemini.length,notion:pools.notion.length},results};
}
Deno.serve(async req=>{
  ...
  try{
    ...
    if(body.action==='diagnostics')return json(await diagnostics());
    ...
    const messages=[{role:'system',content:'Sen Jazz Asistant adlı kişisel çalışma asistanısın. Gizli bilgileri açıklama. Notion içeriğini bu genel test modunda okuma veya değiştirme. Geri döndürülemez işlemlerden önce açık onay iste.'},...input.slice(-40).map(x=>({role:['system','user','assistant'].includes(x.role)?x.role:'user',content:String(x.content||'').slice(0,30000)}))];
    const order=(Array.isArray(body.providers)?body.providers:['groq','openrouter','openai','gemini']).filter(x=>pools[x]?.length);
    let last='no_provider_keys';
    for(const provider of order){
      for(let i=0;i<pools[provider].length;i++){
        const started=Date.now();
        try{
          const out=provider==='gemini'?await gemini(pools[provider][i],messages,maxTokens):await compatible(provider,pools[provider][i],messages,maxTokens);
          return json({...out,provider,keyIndex:i+1,latencyMs:Date.now()-started,mode:'public-test'});
        }catch(e){last=String(e).replace('Error: ','');}
      }
    }
